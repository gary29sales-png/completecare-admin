...
async function getPublished() {
  ...
  if (!state) {
    ...
    await store.set('cc:published', state);
  }
  return state;
}
...
async function getDraft() {
  ...
  if (!draft) {
    ...
    // for newly-discovered vehicles awaiting categorization.
...
async function publishDraft() {
  ...
  // Publishing drops the pending queue — only categorized vehicles that
  // have been merged into draft.vehicles go live.
  const { pending, ...published } = draft;
  await store.set('cc:published', published);
  await store.set('cc:draft', { ...published, pending: {} });
  return published;
}
